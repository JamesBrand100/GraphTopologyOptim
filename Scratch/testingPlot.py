...
torch.manual_seed(0)
num_nodes = 10
max_dist = 0.5     # maximum direct-hop distance; larger needs intermediary
beam_budget = 1.0  # sum of allocations per source
lr = 0.1
epochs = 200
...
src_indices = [0, 1]  # two sources
dst_indices = [2, 3]  # matching destinations
inter_indices = [i for i in range(num_nodes) if i not in src_indices + dst_indices]
demands = { (0,2): 1.0, (1,3): 0.5 }  # demand sizes
