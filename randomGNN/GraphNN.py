...
from torch_geometric.nn import Sequential, GCNConv, NNConv, GENConv, GATConv
...
class GCNNetwork(nn.Module):
    ...
    def _build_gcn_branch(self, in_channels, hidden_channels, out_dim, num_gcn_layers=5, num_mlp_layers=3):
        layers = nn.ModuleList()

        # --- GCN Layers ---
        for i in range(num_gcn_layers):
            input_dim = in_channels if i == 0 else hidden_channels
            layers.append(GATConv(input_dim, hidden_channels, edge_dim=1))
    ...
    def forward(self, x):
        ...
        resTopologyOut = 0*topologyOut

        for i, layer in enumerate(self.topologyNetwork[1:]):
            if isinstance(layer,GCNConv) or isinstance(layer,GENConv) or isinstance(layer,GATConv):
                #skip every 5 connections 
                if(i%3 == 0): 
                    topologyOut += resTopologyOut
                    resTopologyOut = topologyOut
        ...
        resTrafficOut = 0*trafficOut

        for i, layer in enumerate(self.trafficNetwork[1:]):
            if isinstance(layer,GCNConv) or isinstance(layer,GENConv) or isinstance(layer,GATConv):
                if(i%3 == 0): 
                    trafficOut += resTrafficOut
                    resTrafficOut = trafficOut
...
#loss_fn = nn.MSELoss() 
optimizer = optim.Adam(predictorNetwork.parameters(), lr=1e-3)
